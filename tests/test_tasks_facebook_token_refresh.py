"""
Tests for Phase 24's Facebook wiring into app/tasks.py's reactive
TokenExpiredError -> refresh -> retry-once path in publish_job
(_handle_token_expired, Phase 21). Unlike Twitter, no new code was needed in
app/tasks.py for this: "facebook" was already registered in
_TOKEN_REFRESH_MODULES_BY_PLATFORM (pointing at app/publishers/meta.py,
Phase 23), so wiring app/publishers/facebook.py's publish() to raise
TokenExpiredError on a Graph code=190 error (see test_publisher_facebook.py)
was enough to make the existing generic path work. These tests confirm that
end-to-end via publish_job, the same way test_tasks_twitter_token_refresh.py
does for Twitter. Called directly (no Celery worker/broker) against the
throwaway SQLite DB from tests/conftest.py; the facebook publisher and
...
CREDENTIALS = {"page_id": "page-1", "page_token": "old-page-tok", "page_name": "Main Page", "user_token": "old-user-tok"}
NEW_CREDENTIALS = {
    "page_id": "page-1",
...
def _make_account(db_session, credentials, is_active=True):
    account = Account(platform="facebook", name="Main Page", credentials=credentials, is_active=is_active)
...
def _make_job(db_session, account_id):
    job = Job(platform="facebook", payload={"text": "hi"}, account_id=account_id, status=JobStatus.QUEUED)
    db_session.add(job)
    db_session.commit()
...
class TestTokenExpiredRetry:
    def test_refresh_then_retry_succeeds(self, db_session, monkeypatch):
        ...
        fake_publisher, calls = _sequenced_publisher(
            [TokenExpiredError("expired"), {"platform": "facebook", "external_id": "page-1_999"}]
        )
        monkeypatch.setitem(tasks._PUBLISHERS_BY_PLATFORM, "facebook", fake_publisher)
        ...
        assert job.external_id == "page-1_999"
    ...
    def test_no_account_cannot_be_refreshed_falls_back_to_normal_backoff(self, db_session, monkeypatch, fake_retry):
        job = Job(platform="facebook", payload={"text": "hi"}, account_id=None, status=JobStatus.QUEUED)
        db_session.add(job)
        db_session.commit()
        db_session.refresh(job)
